/**
 * Route derivation for the object-page catch-all (`src/pages/[...objectPage].astro`)
...
export type ObjectPageRoutesInput = {
   ...
   * .astro file under src/pages (relative './about.astro' or absolute
   * '/src/pages/about.astro' both work).
   */
...
const routeFileToPath = (filePath: string): string | undefined => {
  ...
  const withoutPrefix = filePath.replace(/^\.?\/?(src\/pages\/)?/, '');

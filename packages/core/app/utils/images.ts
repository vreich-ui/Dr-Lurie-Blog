import { astroAssetsOptimizer } from './images-optimization';
...
export const findImage = async (
  ...
  // W14 T14.1: `~/assets/**` now resolves per site (see site-astro-config.ts's
  // alias table), so the glob's keys are that site's real paths — no longer the
  // one hardwired `/src/assets/...` this used to string-swap in. Match on the
  // `/assets/images/...` tail instead, which is identical for every tenant and
  // unambiguous inside one build.
  const tail = imagePath.replace(/^~\//, '/');
  const key = Object.keys(images).find((candidate) => candidate.endsWith(tail));

  return key && typeof images[key] === 'function'
    ...
    images.map(async (image) => {
      if (image?.url) {
        ...
        if (
          ...
          (resolvedImage.startsWith('http://') || resolvedImage.startsWith('https://') || resolvedImage.startsWith('/'))
        ) {
          _image = {
            src: resolvedImage,

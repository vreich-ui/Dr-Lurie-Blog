...
const getContentBlockPayloadText = (payload: unknown) => {
  const directText = nonEmptyText(payload);
  if (directText) return directText;
  if (!isRecordValue(payload)) return undefined;

  return (
    nonEmptyText(payload.markdown) ??
    nonEmptyText(payload.content) ??
    nonEmptyText(payload.body) ??
    nonEmptyText(payload.text)
  );
};

const hasImportableContentBlockBody = (input: ContentSourceV1) =>
  input.content?.blocks?.some((block) => getContentBlockPayloadText(block.payload) !== undefined) ?? false;
...
const validateAdminPublishDraftInput = (input: ContentSourceV1) => {
  ...
  if (
    !hasAdminImportableText(payload?.content, payload?.markdown, input.editorial?.draft_markdown) &&
    !hasImportableContentBlockBody(input)
  ) {
    issues.push({
      path: ['publication', 'publish_payload', 'content'],
      message:
        'Admin-publish drafts require publication.publish_payload.content, publication.publish_payload.markdown, editorial.draft_markdown, or markdown text in content.blocks.',
    });
  }

...
const signPayload = (encodedPayload, secret) => createHmac('sha256', secret).update(`${tokenVersion}.${encodedPayload}`).digest('base64url');
...
export const signUploadSessionToken = (payload, secret = getUploadSessionTokenSecret()) => {
    if (!secret)
        throw new Error('Upload session token signing is not configured.');
    const encodedPayload = base64UrlJson(payload);
    const signature = signPayload(encodedPayload, secret);
    return `${tokenVersion}.${encodedPayload}.${signature}`;
};
export const validateUploadSessionToken = ({ token, expected, nowMs = Date.now(), secret = getUploadSessionTokenSecret(), }) => {
    if (!secret)
        return { ok: false, statusCode: 500, error: 'Upload session token validation is not configured.' };
    const [version, encodedPayload, signature, extra] = token.split('.');
    if (version !== tokenVersion || !encodedPayload || !signature || extra !== undefined) {
        return { ok: false, statusCode: 401, error: 'Invalid upload token.' };
    }
    ...
    if (payload.expiresAt <= nowMs)
        return { ok: false, statusCode: 401, error: 'Upload token has expired.' };
    if (payload.sessionId !== expected.sessionId ||
        ...
        return { ok: false, statusCode: 401, error: 'Upload token does not match this upload session.' };
    }

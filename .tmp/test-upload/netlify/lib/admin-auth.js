...
export const parseAdminEmails = (value) => (value ?? '').split(',').map(normalizeEmail).filter(Boolean);
export const isAdminEmail = (email, adminEmails) => {
    if (!email)
        return false;
    const normalizedEmail = normalizeEmail(email);
    return adminEmails.some((adminEmail) => normalizeEmail(adminEmail) === normalizedEmail);
};
const getClaimString = (claims, keys) => {
...
const getEmailFromClaims = (claims) => {
    ...
    if (user && typeof user === 'object') {
        return getClaimString(user, ['email', 'email_address', 'emailAddress']);
    }
    return undefined;
};
const getEmailFromClerkUser = async (userId, secretKey) => {
...
export const getAdminStateFromEvent = async (event) => {

...
const updateDraftFrontmatter = (markdown, draft) => {
    ...
    let sawDraft = false;
    const nextLines = lines.reduce((accumulator, line) => {
        if (/^draft\s*:/i.test(line.trim())) {
            sawDraft = true;
            if (draft)
                accumulator.push('draft: true');
            return accumulator;
        }
...
const githubRequest = async (path, token, init = {}) => {
    ...
    if (!response.ok) {
        ...
        throw new TogglePublishError(response.status === 401 || response.status === 403 ? 403 : 500, `GitHub API ${response.status} for ${path}: ${body}`);
    }
...
export const handler = async (event) => {
    ...
    try {
        ...
        const file = await githubRequest(`/repos/${repo}/contents/${encodedPath}?ref=${encodeURIComponent(branch)}`, token);
        ...
        const result = await githubRequest(`/repos/${repo}/contents/${encodedPath}`, token, {
            method: 'PUT',
            body: JSON.stringify({
                branch,

...
const draftWorkflowStatuses = new Set(workflowStatuses.filter((status) => status === 'pending' || status === 'in_progress'));
...
const slugify = (value) => value
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
const loadRecord = async (store, requestId) => {
...
const getPublishPayload = (input) => input.publication?.publish_payload;
const hasRequiredDraftFields = (input) => {
...
const withDraftPublication = (input, previousInput) => {
    const incomingPublication = input.publication ?? {};
    const previousPublication = previousInput?.publication ?? {};
    const incomingPayload = incomingPublication.publish_payload ?? {};
...
const getLockExpirationMs = (expiresAt) => {
    const expiresAtMs = Date.parse(expiresAt);
    return Number.isFinite(expiresAtMs) ? expiresAtMs : 0;
};
const missingLockTokenResponse = () => jsonResponse(400, {
...
const lockExpiredResponse = (lock) => jsonResponse(423, {
    error: 'lock_expired',
...
const lockMismatchResponse = (lock) => jsonResponse(423, {
...
const validateActiveLock = (record, lockToken) => {
    ...
    if (record.lock.token !== lockToken)
        return lockMismatchResponse(record.lock);
    return undefined;
};
export const saveAdminJsonDraft = async (store, body) => {
    ...
    if (body.request_id) {
        if (!body.lock_token)
            return missingLockTokenResponse();

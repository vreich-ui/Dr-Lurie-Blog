...
import { parseContentSourceV1 } from '../../src/schema/schema-v1.js';
const jsonHeaders = {
    'Content-Type': 'application/json',
    'Cache-Control': 'no-store',
};
export const allowedAgents = new Set(allowedAgentNames);
const allowedStatuses = new Set(workflowStatuses);
...
const verifyPublishKey = (event, action) => {
...
const stageIndexKey = (nextAgent, requestId) => `workflows/index/by-stage/${nextAgent}/${requestId}`;
const statusIndexKey = (status, requestId) => `workflows/index/by-status/${status}/${requestId}`;
...
const parseAgentName = (value) => {
    if (value === null || value === undefined)
        return undefined;
    if (typeof value !== 'string' || !allowedAgents.has(value))
        return undefined;
    return value;
};
const parseOptionalAgentName = (value, fieldName) => {
    if (value === null)
    ...
    return { ok: true, value: agentName };
};
const parseRequiredAgentName = (value) => {
    const agentName = parseAgentName(value);
    if (!agentName)
        return undefined;
    return agentName;
...
const parseOptionalWorkflowStatus = (value) => {
    if (value === undefined)
        return { ok: true, value: undefined };
    const status = parseWorkflowStatus(value);
    if (!status)
        return { ok: false, error: 'Invalid workflow_status.' };
    return { ok: true, value: status };
};
const requireStringField = (body, fieldName) => {
    const value = body[fieldName];
    if (!value)
        return jsonResponse(400, { action: body.action, error: `${fieldName} is required.` });
    return undefined;
};
const getLeaseSeconds = (body) => body.lease_seconds ?? DEFAULT_LOCK_LEASE_SECONDS;
const addSecondsIso = (fromMs, seconds) => new Date(fromMs + seconds * 1000).toISOString();
const getLockExpirationMs = (lock) => {
    const expiresAtMs = Date.parse(lock.expires_at);
    return Number.isFinite(expiresAtMs) ? expiresAtMs : 0;
};
const getLockTimestampDiagnostics = (lock, nowMs) => {
...
const isLockActive = (lock, nowMs) => Boolean(lock && getLockExpirationMs(lock) > nowMs);
const lockExpiredResponse = (body, lock, nowMs = Date.now()) => jsonResponse(423, {
    action: body.action,
    error: 'lock_expired',
    lock_expired: true,
    ...(lock ? { diagnostics: getLockTimestampDiagnostics(lock, nowMs) } : {}),
});
const validateMutationLock = (record, body) => {
    const nowMs = Date.now();
    if (!body.lock_token || !record.lock)
        return lockExpiredResponse(body, record.lock, nowMs);
    if (record.lock.token !== body.lock_token)
        return lockExpiredResponse(body, record.lock, nowMs);
    if (!isLockActive(record.lock, nowMs))
        return lockExpiredResponse(body, record.lock, nowMs);
    return undefined;
};
...
const getCommitMetadata = (body) => {
    const metadata = { ...(body.commit_metadata ?? {}) };
...
const loadRecord = async (store, requestId, options) => {
    const value = await store.get(recordKey(requestId), options);
    if (!value)
        return undefined;
    return JSON.parse(value);
};
const isStrongConsistencyUnavailableError = (error) => {
    if (!error || typeof error !== 'object')
        return false;
    const message = 'message' in error && typeof error.message === 'string' ? error.message : '';
    const normalizedMessage = message.toLowerCase();
    return (normalizedMessage.includes('strong') &&
...
const hasActiveMatchingLockToken = (record, lockToken, nowMs = Date.now()) => {
    return Boolean(record.lock?.token === lockToken && isLockActive(record.lock, nowMs));
};
const isPreferredLockTokenCandidate = (candidate, current, lockToken, nowMs = Date.now()) => {
    if (!current)
        return true;
    ...
    if (candidateHasActiveMatchingLock !== currentHasActiveMatchingLock)
        return candidateHasActiveMatchingLock;
    const candidateHasMatchingLock = candidate.lock?.token === lockToken;
    const currentHasMatchingLock = current.lock?.token === lockToken;
    if (candidateHasMatchingLock !== currentHasMatchingLock)
        return candidateHasMatchingLock;
    ...
    if (candidateExpiresAtMs !== currentExpiresAtMs)
        return candidateExpiresAtMs > currentExpiresAtMs;
    return candidate.history.length > current.history.length;
};
const loadRecordForLockToken = async (store, requestId, lockToken) => {
    let preferredRecord;
...
const checkoutDiagnostics = (requestId, diagnostics) => ({
    request_id: requestId,
    record_key: recordKey(requestId),
    ...diagnostics,
});
const loadRecordForCheckout = async (store, requestId) => {
    const nullReadAttempts = [];
    ...
    for (let attempt = 1; attempt <= CHECKOUT_NOT_FOUND_STABILIZATION_ATTEMPTS; attempt += 1) {
        ...
        try {
            ...
            if (!isStrongConsistencyUnavailableError(error))
                throw error;
        }
...
const agentOutputKeys = (record) => Object.keys(record.agent_outputs);
const hasAgentOutput = (record, agentName) => Boolean(record.agent_outputs[agentName]);
const isPreferredAgentOutputCandidate = (candidate, current, requiredAgentName) => {
    if (!current)
        return true;
    if (requiredAgentName) {
        ...
        if (candidateHasRequiredOutput !== currentHasRequiredOutput)
            return candidateHasRequiredOutput;
    }
    if (candidate.version !== current.version)
        return candidate.version > current.version;
    const candidateOutputCount = agentOutputKeys(candidate).length;
    const currentOutputCount = agentOutputKeys(current).length;
    if (candidateOutputCount !== currentOutputCount)
        return candidateOutputCount > currentOutputCount;
    return candidate.history.length > current.history.length;
};
const loadLatestRecordForAgentOutputs = async (store, requestId, initialRecord, requiredAgentName) => {
    ...
    for (let attempt = 0; attempt < WORKFLOW_MUTATION_MAX_RETRIES; attempt += 1) {
        ...
        if (requiredAgentName && latestRecord && hasAgentOutput(latestRecord, requiredAgentName))
            break;
...
const preserveAgentOutputs = (record, sourceRecord) => ({
    ...record,
    agent_outputs: {
...
const saveWorkflowMutationRecord = async (store, previousRecord, nextRecord) => {
...
const saveRecordIfVersionUnchanged = async (store, expectedRecord, nextRecord) => {
    const latestRecord = await loadLatestRecordForAgentOutputs(store, expectedRecord.request_id);
    if (!latestRecord)
        return { saved: false, notFound: true };
    if (latestRecord.version !== expectedRecord.version)
        return { saved: false, latestRecord };
    const recordToSave = preserveAgentOutputs(nextRecord, latestRecord);
    await saveRecord(store, recordToSave);
    await updateIndexes(store, latestRecord, recordToSave);
    return { saved: true, record: recordToSave };
};
const hasHistoryAction = (record, action) => {
    return record.history.some((entry) => entry.action === action);
};
const isPreferredCheckinCandidate = (candidate, current) => {
    if (candidate.version !== current.version)
        return candidate.version > current.version;
    if (!hasHistoryAction(current, 'mark_published') && hasHistoryAction(candidate, 'mark_published'))
        return true;
    return candidate.history.length > current.history.length;
};
const loadLatestRecordForCheckin = async (store, requestId) => {
    let latestRecord;
    for (let attempt = 0; attempt < WORKFLOW_MUTATION_MAX_RETRIES; attempt += 1) {
        const candidate = await loadRecord(store, requestId);
...
const loadLatestRecordForGetRequest = async (store, requestId) => {
    let latestRecord;
    for (let attempt = 0; attempt < WORKFLOW_MUTATION_MAX_RETRIES; attempt += 1) {
        const candidate = await loadRecord(store, requestId);
        if (candidate && (!latestRecord || isPreferredCheckinCandidate(candidate, latestRecord))) {
...
const saveCheckinIfLatestRecordUnchanged = async (store, expectedRecord, nextRecord) => {
...
const deleteBlob = async (store, key) => {
...
const updateIndexes = async (store, previousRecord, nextRecord) => {
...
const listIndexRequestIds = async (store, prefix) => {
...
const validateBodyFields = (body) => {
    ...
    if (!currentStage.ok)
        return jsonResponse(400, { action: body.action, error: currentStage.error });
    const nextAgent = parseOptionalAgentName(body.next_agent, 'next_agent');
    if (!nextAgent.ok)
        return jsonResponse(400, { action: body.action, error: nextAgent.error });
    const workflowStatus = parseOptionalWorkflowStatus(body.workflow_status);
    if (!workflowStatus.ok)
        return jsonResponse(400, { action: body.action, error: workflowStatus.error });
    return undefined;
};
const requireRequestId = (body) => {
    if (!body.request_id)
        return jsonResponse(400, { action: body.action, error: 'request_id is required.' });
    return undefined;
};
const isRecordValue = (value) => Boolean(value && typeof value === 'object');
...
const finalArticleStringKeyMappings = [
    { from: 'title', to: 'title' },
    { from: 'slug', to: 'slug' },
...
const hasAdminImportableText = (...values) => values.some((value) => nonEmptyText(value) !== undefined);
const getAdminMarkdownBlockText = (payload) => {
    if (typeof payload === 'string')
        return payload;
    if (isRecordValue(payload) && typeof payload.markdown === 'string')
        return payload.markdown;
    return undefined;
};
const getAdminMarkdownBlocksText = (input) => input.content?.blocks
    ?.filter((block) => block.block_type === 'markdown')
    .map((block) => getAdminMarkdownBlockText(block.payload))
    .filter((value) => nonEmptyText(value) !== undefined)
    .join('\n\n');
const validateAdminPublishDraftInput = (input) => {
    ...
    if (!hasAdminImportableText(payload?.slug, input.content?.title)) {
        issues.push({
            ...
            message: 'Admin-publish drafts require publication.publish_payload.slug or enough content.title text to compute one.',
        });
    }
    ...
    if (!hasAdminImportableText(payload?.markdown, payload?.content, input.editorial?.draft_markdown, getAdminMarkdownBlocksText(input))) {
        issues.push({
            path: ['publication', 'publish_payload', 'content'],
            message: 'Admin-publish drafts require publication.publish_payload.markdown, publication.publish_payload.content, editorial.draft_markdown, or content.blocks markdown body text.',
        });
    }
    return issues;
};
const getAdminPublishDraftValidationFailure = (input, body) => {
...
export const createRequest = async (store, body) => {
    ...
    if (adminPublishDraftValidationFailure)
        return adminPublishDraftValidationFailure;
    const existingRecord = await loadRecord(store, body.request_id);
    if (existingRecord) {
    ...
    const currentAgent = parseOptionalAgentName(body.current_agent ?? body.current_stage ?? parsedInput.data.workflow?.current_agent, 'current_agent');
    if (!currentAgent.ok)
        return jsonResponse(400, { action: body.action, error: currentAgent.error });
    const nextAgent = parseOptionalAgentName(body.next_agent !== undefined ? body.next_agent : parsedInput.data.workflow?.next_agent, 'next_agent');
    if (!nextAgent.ok)
        return jsonResponse(400, { action: body.action, error: nextAgent.error });
...
export const getRequest = async (store, body) => {
    const missingRequestId = requireRequestId(body);
    if (missingRequestId)
        return missingRequestId;
    const record = await loadLatestRecordForGetRequest(store, body.request_id);
    if (!record)
        return jsonResponse(404, { action: body.action, not_found: true });
    return jsonResponse(200, { action: body.action, record });
};
const listPendingRequests = async (store, body) => {
...
export const patchAgentOutput = async (store, body) => {
    const missingRequestId = requireRequestId(body);
    if (missingRequestId)
        return missingRequestId;
    ...
    if (existingVersion !== body.expected_agent_version) {
        if (existingOutput?.expected_agent_version === body.expected_agent_version &&
...
const completionAlreadyReflected = (record, agentName, body) => {
    if (!record.completed_agents.includes(agentName))
        return false;
    if (record.failed_agents.includes(agentName))
        return false;
    const currentStage = parseOptionalAgentName(body.current_stage, 'current_stage');
    const nextAgent = parseOptionalAgentName(body.next_agent, 'next_agent');
    const workflowStatus = parseOptionalWorkflowStatus(body.workflow_status);
    if (!currentStage.ok || !nextAgent.ok || !workflowStatus.ok)
        return false;
    if (currentStage.value !== undefined && record.current_stage !== currentStage.value)
        return false;
    if (nextAgent.value !== undefined && record.next_agent !== nextAgent.value)
        return false;
    if (workflowStatus.value !== undefined && record.workflow_status !== workflowStatus.value)
        return false;
    if (body.needs_review !== undefined && record.needs_review !== body.needs_review)
        return false;
    if (body.last_error !== undefined && record.last_error !== body.last_error)
        return false;
    return true;
};
const loadRecordAtExpectedVersion = async (store, body, previousRecord) => {
    ...
    for (let retryIndex = 0; retryIndex < MARK_COMPLETE_STALE_READ_MAX_RETRIES; retryIndex += 1) {
        ...
        const retryRecord = await loadRecord(store, body.request_id);
        if (!retryRecord)
            return { notFound: true };
        latestRecord = retryRecord;
        if (latestRecord.version >= body.expected_record_version)
            break;
    }
    return { record: latestRecord };
};
const stabilizeFinalArticleCompletionSource = async (store, requestId, record) => {
    if (hasAgentOutput(record, 'final_article'))
        return { record };
...
export const markAgentComplete = async (store, body) => {
    const missingRequestId = requireRequestId(body);
    if (missingRequestId)
        return missingRequestId;
    ...
    if (agentName === 'final_article') {
        ...
        if ('error' in stabilizedSource)
            return stabilizedSource.error;
...
export const checkoutRequest = async (store, body) => {
    const missingRequestId = requireRequestId(body);
    if (missingRequestId)
        return missingRequestId;
    ...
    if (missingOwnerLabel)
        return missingOwnerLabel;
    const requestId = body.request_id;
...
export const refreshLock = async (store, body) => {
    const missingRequestId = requireRequestId(body);
    if (missingRequestId)
        return missingRequestId;
    const missingLockToken = requireStringField(body, 'lock_token');
    if (missingLockToken)
        return missingLockToken;
    const requestId = body.request_id;
    const previousRecord = (await loadRecordForLockToken(store, requestId, body.lock_token)).record;
    if (!previousRecord)
        return jsonResponse(404, { action: body.action, not_found: true });
    if (!previousRecord.lock)
        return jsonResponse(409, { action: body.action, error: 'No lock is currently held.' });
...
export const checkinRequest = async (store, body) => {
    const missingRequestId = requireRequestId(body);
    if (missingRequestId)
        return missingRequestId;
    const missingLockToken = requireStringField(body, 'lock_token');
    if (missingLockToken)
        return missingLockToken;
    for (let attempt = 0; attempt < WORKFLOW_MUTATION_MAX_RETRIES; attempt += 1) {
        const latestRecord = await loadLatestRecordForCheckin(store, body.request_id);
        if (!latestRecord)
            return jsonResponse(404, { action: body.action, not_found: true });
        if (!latestRecord.lock)
        ...
        if (saveResult.saved)
            return jsonResponse(200, { action: body.action, record: saveResult.record });
        if ('notFound' in saveResult)
            return jsonResponse(404, { action: body.action, not_found: true });
    }
    return jsonResponse(409, { action: body.action, conflict: true });
};
const forceUnlock = async (store, body) => {
    const missingRequestId = requireRequestId(body);
    if (missingRequestId)
        return missingRequestId;
...
const getPublishPreconditionFailure = (record) => {
    if (record.workflow_status !== 'completed')
        return 'Workflow must be completed before publishing.';
    if (!record.completed_agents.includes('final_article'))
        return 'final_article must be completed before publishing.';
    if (!record.agent_outputs.final_article)
        return 'final_article output must be present before publishing.';
    if (record.current_stage !== null)
        return 'current_stage must be null before publishing the completed final article.';
    return undefined;
};
const isRecordReadyForPublish = (record, body, nowMs = Date.now()) => {
    ...
    if (!record.agent_outputs.final_article)
        return false;
    if (!body.lock_token || !hasActiveMatchingLockToken(record, body.lock_token, nowMs))
        return false;
    return true;
};
const getPublishReadinessScore = (record, body, nowMs = Date.now()) => {
    let score = 0;
    if (body.expected_record_version === undefined || record.version >= body.expected_record_version)
        score += 1;
    if (record.workflow_status === 'completed')
...
const isPreferredPublishCandidate = (candidate, current, body, nowMs = Date.now()) => {
    if (!current)
        return true;
    ...
    if (candidateHasFinalOutput !== currentHasFinalOutput)
        return candidateHasFinalOutput;
...
const loadRecordReadyForPublish = async (store, body) => {
    const requestId = body.request_id;
    let preferredRecord;
...
export const markPublished = async (store, body) => {
    const missingRequestId = requireRequestId(body);
    if (missingRequestId)
        return missingRequestId;
    for (let attempt = 0; attempt < WORKFLOW_MUTATION_MAX_RETRIES; attempt += 1) {
        ...
        const nextRecord = preserveAgentOutputs({
            ...latestRecord,
            updated_at: timestamp,
...
const handleAction = async (store, body) => {
    switch (body.action) {
        case 'create_request':

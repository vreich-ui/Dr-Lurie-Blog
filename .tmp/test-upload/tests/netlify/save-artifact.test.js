...
import { getArtifactBlobStore, getArtifactIndexBlobStore, setNetlifyBlobsModuleForTesting, } from '../../netlify/lib/blob-store.js';
const publishSecret = 'artifact-test-secret';
const validJpegBytes = Buffer.from('', 'base64');
...
const makeBaseInput = (requestId) => ({
    requestId,
    artifactKind: 'image',
    contentType: 'image/png',
    filename: 'hero.png',
});
const listArtifactsWithMcp = async (requestId) => {
...
const postChunkedJpeg = async ({ requestId, bytes, chunkSizeBytes, expectedSizeBytes = bytes.byteLength, expectedSha256 = sha256(bytes), }) => {
    const clientUploadId = randomUUID();
    const totalChunks = Math.ceil(bytes.byteLength / chunkSizeBytes);
    let response;
    for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex += 1) {
        const chunk = bytes.subarray(chunkIndex * chunkSizeBytes, Math.min(bytes.byteLength, (chunkIndex + 1) * chunkSizeBytes));
    ...
    const fakeStore = {
        async set(key, value) {
            ...
            if (key.endsWith('/1') || key.endsWith('/2'))
                hiddenImmediateChunkReads.add(key);
        },
...
test('save-artifact retries stale final artifact readback before saving request index', async () => {
    ...
    const hiddenFinalArtifactReads = new Set();
    const finalArtifactArrayBufferReads = new Map();
    const deletedFinalArtifactKeys = [];
    const createFakeStore = (values, hideFirstFinalRead) => ({
        async set(key, value) {
            values.set(key, typeof value === 'string' ? value : value instanceof ArrayBuffer ? Buffer.from(value) : Buffer.from(value));
            if (hideFirstFinalRead && key.startsWith('image/'))
                hiddenFinalArtifactReads.add(key);
        },
        async setJSON(key, value) {
            values.set(key, JSON.stringify(value));
        },
        ...
        async del(key) {
            if (hideFirstFinalRead && key.startsWith('image/'))
                deletedFinalArtifactKeys.push(key);
            values.delete(key);
        },
        async list() {
            return { blobs: [...values.keys()].map((key) => ({ key, etag: '' })), directories: [] };
    ...
    try {
        ...
        assert.deepEqual(JSON.parse(indexedReferenceText), response.json.artifact);
    }
    finally {
...
test('save-artifact deletes final artifact blob when readback retries are exhausted', async () => {
    ...
    const artifactStore = {
        async set(key, value) {
            artifactValues.set(key, typeof value === 'string' ? value : value instanceof ArrayBuffer ? Buffer.from(value) : Buffer.from(value));
        },
        async setJSON(key, value) {
        ...
        async get(key, options) {
            ...
            if (value === undefined)
                return null;
            if (key.startsWith('image/') && options?.type === 'arrayBuffer')
                return null;
            if (options?.type === 'arrayBuffer') {
                const bytes = typeof value === 'string' ? Buffer.from(value) : value;
                return bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength);
    ...
    const indexStore = {
        ...
        async get(key) {
            return indexValues.get(key) ?? null;
        },
        async del(key) {
...
test('save-artifact single-shot uploads dedupe by checksum', async () => {
    ...
    const firstArtifact = first.json.artifact;
    ...
    assert.deepEqual(artifactList.blobs.map((blob) => blob.key), [firstArtifact.blobKey]);
    ...
    const indexedReference = indexedReferenceText ? JSON.parse(indexedReferenceText) : null;
...
test('save-artifact accepts a valid JPEG upload with matching expected size and sha256', async () => {
    ...
    const artifactStore = await getArtifactBlobStore({});
    const retrievedBytes = await artifactStore.get(artifact.blobKey, { type: 'buffer' });
...
test('save-artifact accepts valid PNG and WebP uploads', async () => {
    ...
    const pngArtifact = pngResponse.json.artifact;
    const webpArtifact = webpResponse.json.artifact;
...
test('save-artifact saves safe ArtifactReference display fields and rejects unsafe upload fields', async () => {
    ...
    const artifact = validResponse.json.artifact;
    ...
    assert.deepEqual(JSON.parse((await indexStore.get(`by-kind/image/${artifact.sha256}.json`)) || '{}'), expectedPointer);
    assert.deepEqual(JSON.parse((await indexStore.get(`by-request/${encodeURIComponent(requestId)}/image/${artifact.sha256}.json`)) || '{}'), expectedPointer);
...
test('save-artifact chunked uploads collect three chunks by request and client upload before finalizing', async () => {
    ...
    const baseInput = makeBaseInput(requestId);
    const clientUploadId = randomUUID();
    const expectedBytes = await createImageBytes('png');
    const splitIndexes = [Math.ceil(expectedBytes.byteLength / 3), Math.ceil((expectedBytes.byteLength * 2) / 3)];
    const chunkBuffers = [
        expectedBytes.subarray(0, splitIndexes[0]),
        expectedBytes.subarray(splitIndexes[0], splitIndexes[1]),
        expectedBytes.subarray(splitIndexes[1]),
    ];
    ...
    const completedArtifact = completed.json.artifact;
    ...
    assert.deepEqual(completedIndexes.blobs.map((blob) => blob.key), [`request-artifacts/${requestId}/${completedArtifact.sha256}.json`]);
    const completedIndexedReferenceText = await indexStore.get(`request-artifacts/${requestId}/${completedArtifact.sha256}.json`);
    const completedIndexedReference = completedIndexedReferenceText
        ? JSON.parse(completedIndexedReferenceText)
        : null;
    ...
    assert.deepEqual(finalBlobs.blobs.map((blob) => blob.key), [completedArtifact.blobKey]);
...
test('save-artifact finalizes chunked JPEG uploads with matching expected size and sha256', async () => {
    ...
    const artifact = completed.json.artifact;
    assert.equal(artifact.sha256, validJpegSha256);
    assert.equal(artifact.sizeBytes, validJpegBytes.byteLength);
    const artifactStore = await getArtifactBlobStore({});
    const retrievedBytes = await artifactStore.get(artifact.blobKey, { type: 'buffer' });
...
test('save-artifact rejects completed chunked JPEG only after assembled bytes fail validation', async () => {
    ...
    assert.match(String(completed.json.error), /could not be decoded as a valid JPEG/);
    const artifactStore = await getArtifactBlobStore({});
    const indexStore = await getArtifactIndexBlobStore({});
    assert.deepEqual((await artifactStore.list({ prefix: `image/${requestId}/` })).blobs, []);
    assert.deepEqual((await indexStore.list({ prefix: `request-artifacts/${requestId}/` })).blobs, []);
...
test('save-artifact preserves a valid 1-2 KB JPEG chunked upload with exact size and sha256', async () => {
    ...
    const artifactStore = await getArtifactBlobStore({});
    const storedBytes = await artifactStore.get(artifact.blobKey, { type: 'buffer' });
...
test('save-artifact rejects a chunked truncated JPEG and leaves no listed artifact', async () => {
    ...
    const truncatedBytes = Buffer.from([0xff, 0xd8, 0x00, 0x00, 0xff, 0xd9]);
...
test('MCP artifact tools upload bytes and list request references', async () => {
    ...
    const uploadBytes = await createImageBytes('png');

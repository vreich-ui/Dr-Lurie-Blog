...
const mcpRequest = async (method, headers = {}) => {
    const logs = [];
...
test('MCP_HTTP_AUTH_TOKEN unset allows initialize', async () => {
    ...
    assert.equal(body.result?.serverInfo?.name, 'Dr_Lurie_MCP_Server');
});
...
test('correct x-mcp-auth-token allows initialize and tools/list', async () => {
    ...
    assert.equal(initialize.body.result?.serverInfo?.name, 'Dr_Lurie_MCP_Server');

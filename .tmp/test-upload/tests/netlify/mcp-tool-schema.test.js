...
test('save_json_blob_create_request exposes structured content_source.v1 input schema', async () => {
    ...
    assert.equal(inputProperties.record_type.const, 'content_source');
    assert.equal(inputProperties.schema_version.const, 'content_source.v1');
    ...
    const publication = inputProperties.publication;
    const publicationStatus = property(publication, 'publication_status');
...
test('content_source.v1 MCP schema uses concrete workflow and agent-priority section items', async () => {
    ...
    const media = property(inputSchema, 'media');
    const revisionControl = property(inputSchema, 'revision_control');
    const claimItems = property(claims, 'claim_list').items;
    const requirementItems = property(compliance, 'requirements').items;
    const offerItems = property(commercial, 'offers').items;
    const assetItems = property(media, 'image_asset_register').items;
    const revisionItems = property(revisionControl, 'revision_requests').items;
    assert.deepEqual(property(workflow, 'current_agent').enum, [...allowedAgentNames]);
...
test('workflow mutation tools expose lock_token schemas and lock-aware descriptions', async () => {
    ...
    for (const name of [
        ...
        assert.match(String(tool.description), /checkout first/i);
        assert.match(String(tool.description), /check in/i);
    }
});
test('stage mark-complete helpers expose transition fields and document common routing', async () => {
    ...
    for (const [name, transitionText] of expectedTransitions) {
        ...
        assert.match(String(tool.description), new RegExp(transitionText));
    }
    const genericCompleteTool = tools.get('save_json_blob_mark_agent_complete');
    assert.ok(genericCompleteTool);
    assert.deepEqual(property(genericCompleteTool.inputSchema, 'agent_name').enum, [
        ...allowedAgentNames,
    ]);
    assert.deepEqual(property(genericCompleteTool.inputSchema, 'workflow_status').enum, [
        ...workflowStatuses,
    ]);
    const finalTool = tools.get('final_article_mark_complete');
    assert.ok(finalTool);
    assert.match(String(finalTool.description), /workflow_status: "completed"/);
});
test('save_json_blob_publish_scheduled exposes gated scheduled publish inputs', async () => {
    ...
    assert.match(String(tool.description), /publication\.publication_status: scheduled/);
    assert.match(String(tool.description), /mark the workflow published/i);
...
test('save_json_blob_mark_published exposes only workflow-state inputs', async () => {
    ...
    assert.match(String(tool.description), /does not invoke the article publishing endpoint/i);
    const serializedSchema = JSON.stringify(tool);
    assert.equal(serializedSchema.includes('NETLIFY_PUBLISH_SECRET'), false);
    assert.equal(serializedSchema.includes('PUBLISH_SECRET'), false);
});
...
test('artifact MCP tools are registered with precise byte-vs-metadata descriptions', async () => {
    ...
    assert.equal(property(saveArtifact.inputSchema, 'tags').items.maxLength, 40);
    assert.deepEqual(property(saveArtifact.inputSchema, 'artifactKind').enum, [
        'image',
        'pdf',
        'video',
        'doc',
    ...
    assert.match(String(saveArtifact.description), /immediately after creating/i);
    assert.match(String(saveArtifact.description), /store only the returned ArtifactReference/i);
    assert.match(String(saveArtifact.description), /never invent blobKey/i);
    assert.match(String(saveArtifact.description), /preferred\/default artifact path/i);
    assert.match(String(saveArtifact.description), /50-150 KB JPEG\/PNG images should be uploaded in one call/i);
    assert.match(String(property(saveArtifact.inputSchema, 'payload').description), /do not chunk merely because an image is around 50 KB/i);
    assert.match(String(saveArtifact.description), /Writes final artifact bytes/i);
    assert.match(String(saveArtifact.description), /ArtifactReference index/i);
    assert.match(String(saveArtifact.description), /dedup is success/i);
    ...
    assert.deepEqual(property(saveChunk.inputSchema, 'artifactKind').enum, [
        'image',
        'pdf',
        'video',
        'doc',
    ...
    assert.match(String(property(saveChunk.inputSchema, 'totalChunks').description), /For normal 50-150 KB images, use save_artifact/i);
    assert.match(String(property(saveChunk.inputSchema, 'payload').description), /target about 256000 raw bytes per chunk/i);
    ...
    assert.match(String(createSession.description), /chunkSizeBytes=5242880/);
    assert.match(String(createSession.description), /larger binary assets/i);
    ...
    assert.match(String(finalizeSession.description), /Idempotent retries/i);
    assert.ok(property(finalizeSession.inputSchema, 'sessionId'));
    assert.match(String(saveChunk.description), /fallback for artifacts too large for one MCP tool call/i);
    assert.match(String(saveChunk.description), /Do not use this for ordinary 50-150 KB generated web images/i);
    assert.match(String(saveChunk.description), /largest safe chunks/i);
    assert.match(String(saveChunk.description), /store only the final returned ArtifactReference/i);
    assert.match(String(saveChunk.description), /never invent blobKey/i);
    assert.match(String(saveChunk.description), /Writes one chunk blob/i);
    assert.match(String(saveChunk.description), /assembles final artifact bytes/i);
    assert.match(String(saveChunk.description), /complete=false/i);
    assert.match(String(saveChunk.description), /dedup is success/i);
    const listArtifacts = tools.get('list_artifacts_for_request');
    assert.deepEqual(listArtifacts.inputSchema.required, ['requestId']);
    assert.match(String(listArtifacts.description), /Reads the request artifact index only/i);
    assert.match(String(listArtifacts.description), /does not read or write artifact bytes/i);
    ...
    assert.match(String(listByKind.description), /Admin-only/);
    ...
    assert.match(String(searchArtifacts.description), /prefix indexes/);
    ...
    assert.match(String(softDeleteArtifact.description), /soft delete/i);
    const restoreArtifact = tools.get('restore_artifact');
    assert.deepEqual(restoreArtifact.inputSchema.required, ['requestId', 'sha256']);
    assert.match(String(restoreArtifact.description), /restore/i);
    ...
    assert.match(String(migrateArtifacts.description), /one-time artifact-index migration/);
    ...
    assert.match(String(wipeBlobStores.description), /maintenance tool/);
    ...
    assert.match(String(reconcileArtifacts.description), /Admin-only artifact-index correction job/);
    assert.match(String(reconcileArtifacts.description), /corrects stale artifact-index blobKey values/);
    for (const name of [
        'save_artifact',
        'save_artifact_chunk',
        'save_artifact_create_upload_session',
...
test('verify_article_images appears in tools/list with expected input schema', async () => {
    ...
    assert.equal(expectedImages.items.type, 'string');
    assert.equal(JSON.stringify(tool).includes('NETLIFY_PUBLISH_SECRET'), false);
    assert.equal(JSON.stringify(tool).includes('PUBLISH_SECRET'), false);
});

...
import { checkinRequest, handler } from '../../netlify/functions/save-json-blob.js';
...
const postWorkflowAction = async (body) => {
...
const parseRecordResponse = (response) => {
...
test('checkin_request preserves newer published canonical workflow state over a stale completed checkout snapshot', async () => {
    ...
    const checkin = parseRecordResponse(await checkinRequest(staleOnceStore, {
    ...
    assert.deepEqual(fetched.record.history.find((entry) => entry.action === 'mark_published')?.details?.commit_metadata, {
        commit: 'published-checkin-regression-commit',
        articlePath: 'src/data/post/checkin-regression-article.md',
        deployStatus: 'queued',
    });

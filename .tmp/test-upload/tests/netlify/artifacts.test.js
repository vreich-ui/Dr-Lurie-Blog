...
import { ArtifactKind, artifactKindValues, createArtifactBlobKey, createArtifactReference, getArtifactReferenceIssue, isArtifactReference, } from '../../netlify/lib/artifacts.js';
...
test('artifact blob keys fall back safely when request IDs are not path-safe', () => {
    ...
    assert.equal(createArtifactBlobKey({
...
test('artifact blob keys enforce the server artifactKind whitelist and sha256 format', () => {
    ...
    assert.throws(() => createArtifactBlobKey({
...
test('ArtifactReference validation rejects invented media handles and incomplete references', () => {
    ...
    assert.equal(getArtifactReferenceIssue({ ...reference, url: 'https://example.com/deterministic.png' }), 'unexpected top-level keys: url');
    assert.match(getArtifactReferenceIssue({ blobKey: reference.blobKey, sha256: reference.sha256 }) ?? '', /sizeBytes must be a non-negative number/);
    assert.match(getArtifactReferenceIssue({ ...reference, blobKey: reference.blobKey.replace(/^image\//, 'markdown/') }) ?? '', /blobKey must match the server ArtifactReference path format/);
    assert.match(getArtifactReferenceIssue({ ...reference, artifactKind: 'markdown' }) ?? '', /artifactKind must be one of/);
    assert.match(getArtifactReferenceIssue({ ...reference, originalFilename: '../unsafe.png' }) ?? '', /originalFilename must be a filename/);
    assert.match(getArtifactReferenceIssue({ ...reference, label: '<script>' }) ?? '', /label must not contain/);
    assert.match(getArtifactReferenceIssue({ ...reference, tags: ['x'.repeat(41)] }) ?? '', /tags\[0\] must be at most 40/);
    assert.equal(getArtifactReferenceIssue({
        ...reference,
        deletedAtISO: '2026-06-10T00:00:00.000Z',
        deletedBy: 'admin@example.com',
    }), undefined);
    assert.match(getArtifactReferenceIssue({ ...reference, deletedAtISO: 'not-a-date' }) ?? '', /deletedAtISO must be a valid ISO date string/);
    assert.match(getArtifactReferenceIssue({ ...reference, deletedBy: '<admin>' }) ?? '', /deletedBy must not contain/);
});
const createFakeArtifactStore = (initialValues = {}) => {
...
const makeImageReference = (requestId, bytes = Buffer.from('image bytes'), filename = 'hero.jpg') => createArtifactReference({
...
test('reconcileArtifactReference normalizes stale blobKeys and corrects artifact-index JSON', async () => {
    ...
    const staleReference = { ...reference, blobKey: `artifacts/${reference.blobKey}` };
    ...
    const result = await reconcileArtifactReference(staleReference, store, indexStore, {
        logger: { warn: (...args) => loggedCorrections.push(args) },
    });
    assert.equal(result.status, 'found');
    assert.equal(result.blobKey, reference.blobKey);
...
test('reconcileImageArtifactReference recovers a blob stored under duplicated artifacts/image prefix', async () => {
    ...
    assert.equal(values.has(`request-artifacts/duplicated-prefix/${reference.sha256}.json`), false, 'legacy prefixed keys should not be written back as invalid ArtifactReference blobKeys');
});

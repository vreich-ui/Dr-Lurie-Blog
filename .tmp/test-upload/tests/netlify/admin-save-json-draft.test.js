    ...
    publication: {
        ...
        publish_payload: {
            ...
            ...(overrides.publish_payload && typeof overrides.publish_payload === 'object'
                ? overrides.publish_payload
                : {}),
        },
    },
    ...(overrides.input && typeof overrides.input === 'object' ? overrides.input : {}),
});
const checkedOutRecord = (input = validInput()) => ({
...
test('admin JSON draft save keeps the existing lock after a successful checked-out update', async () => {
    ...
    const body = parseBody(response);
    const record = body.record;
    const persisted = JSON.parse((await store.get(recordKey(existing.request_id))) ?? '{}');
    assert.equal(response.statusCode, 200);

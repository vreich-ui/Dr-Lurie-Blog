...
const contentSourceInput = (requestId, body = 'Initial publish smoke body.') => ({
...
const callToolRaw = async (name, args) => {
...
const installGitHubPublishMock = (slug = 'admin-publish-e2e-smoke', commitSha = 'published-smoke-commit') => {
    ...
    globalThis.fetch = (async (input, init) => {
        ...
        if (url.endsWith('/git/blobs') && method === 'POST') {
            blobWrites.push(JSON.parse(String(init?.body)));
...
test('admin publish pseudo-E2E propagates lock through draft save, publish, mark_published, and checkin', async () => {
    ...
    const missingLockBody = parseJsonBody(missingLockResponse);
    ...
    const savedDraftBody = parseJsonBody(savedDraftResponse);
    ...
    assert.equal(finalCompleteRecord.input.publication?.publish_payload?.seoDescription, 'Final article SEO description.');
    assert.equal(finalCompleteRecord.input.publication?.publish_payload?.featuredImage, 'https://kugelmedia.netlify.app/drlurieblog/final-article.jpg');
    ...
    const fetchedDraftRecord = fetchedDraftResult.record;
    ...
    try {
        ...
        const publishBody = parseJsonBody(publishResponse);
        ...
        assert.equal(github.blobWrites.some((write) => write.content.includes('Edited draft body that should be published.')), false);
...
test('scheduled publish tool publishes due scheduled records and returns not-due reasons for future records', async () => {
    ...
    const scheduledInput = contentSourceInput(requestId, 'Scheduled publish smoke body.');
    ...
    const checkedOutRecord = checkoutResult.record;
    ...
    const completeRecord = completeResult.record;
    ...
    try {
        ...
        const publishedRecord = scheduledResult.record;
        assert.equal(publishedRecord.workflow_status, 'published');
        assert.equal(scheduledResult.publish_result?.commit, 'scheduled-published-commit');

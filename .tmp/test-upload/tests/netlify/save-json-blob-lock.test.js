...
import { checkoutRequest, createRequest, getRequest, markAgentComplete, markPublished, patchAgentOutput, refreshLock, } from '../../netlify/functions/save-json-blob.js';
...
const createAndCheckout = async (store, requestId, leaseSeconds = 300) => {
...
test('refresh_lock recovers when a stale expired-lock snapshot is followed by an active matching lock', async () => {
    ...
    assert.notEqual(body.lock_expired, true);
    assert.equal(body.record?.lock?.token, lock.token);
    assert.ok(Date.parse(body.record?.lock?.expires_at ?? '') > Date.parse(lock.expires_at), 'refresh should extend the active matching lock rather than fail on the stale expired snapshot');
    assert.equal(returnedExpiredSnapshot, true);
});
test('final_article completion preserves output when first completion read is stale without final_article output', async () => {
    ...
    assert.deepEqual(completeBody.record?.agent_outputs.final_article, patchedRecord?.agent_outputs.final_article, 'completion response should preserve the previously written final_article output');
    ...
    assert.deepEqual(getBody.record?.agent_outputs.final_article, patchedRecord?.agent_outputs.final_article, 'durable get_request should preserve the previously written final_article output');
...
test('mark_published stabilizes when first publish read omits visible final_article output', async () => {
    ...
    assert.deepEqual(visibleGetBody.record?.agent_outputs.final_article, completedRecord?.agent_outputs.final_article, 'get_request should be able to see final_article output before publish');
    const stalePublishSnapshot = {
        ...completedRecord,
        agent_outputs: {},
    };
    const originalGet = store.get.bind(store);
    ...
    assert.deepEqual(publishBody.record?.agent_outputs.final_article, completedRecord?.agent_outputs.final_article, 'publish response should preserve final_article output after a stale first read');
    ...
    assert.deepEqual(durableGetBody.record?.agent_outputs.final_article, completedRecord?.agent_outputs.final_article, 'durable get_request should preserve final_article output after publish');
});

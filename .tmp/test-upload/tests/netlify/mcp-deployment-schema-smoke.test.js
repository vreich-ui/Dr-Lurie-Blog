...
const callToolOk = async (name, args) => {
...
test('MCP initialize and tools/list expose deployed connector schema requirements', async () => {
    ...
    assert.equal(initialize.result?.serverInfo?.name, 'Dr_Lurie_MCP_Server');
    ...
    assert.ok(markPublished.inputSchema.properties?.expected_record_version, 'Expected save_json_blob_mark_published.inputSchema.properties.expected_record_version to exist.');
});
test('save_json_blob_mark_published forwards expected_record_version to the backend action', async () => {
    ...
    const checkoutRecord = checkout.record;
    ...
    const publishedRecord = published.record;

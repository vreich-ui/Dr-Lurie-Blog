...
const indexAfter = (source, pattern, startIndex) => {
...
test('admin publish flow re-fetches workflow state and requires lock before publishing', async () => {
    ...
    const artifactReferenceIndex = indexAfter(source, '...(artifactReferences.length ? { artifactReferences } : {}),', publishIndex);
...
test('admin publish flow sends lock_token to mark_published and checks in after success', async () => {
    ...
    const expectedVersionIndex = indexAfter(source, 'expected_record_version: latestWorkflowRecord?.version,', requestIdIndex);
    const lockTokenIndex = indexAfter(source, 'lock_token: lockToken,', expectedVersionIndex);
    const commitMetadataIndex = indexAfter(source, 'commit_metadata: { commit, articlePath: publishedPath },', lockTokenIndex);
    ...
    assert.ok(expectedVersionIndex > requestIdIndex, 'mark_published payload must include expected_record_version from the latest workflow state.');
...
test('admin publish JSON import filters unreadable artifact references before selection', async () => {
    ...
    const selectedIndex = indexAfter(source, '...importedReadableArtifactReferences.map(createArtifactSelectedImage),', checkIndex);
    const warningIndex = indexAfter(source, 'formatArtifactReselectionMessage(importedFailedArtifactReferences)', selectedIndex);
...
test('admin publish payload omits stale existing featured paths for selected publishable images', async () => {
    ...
    const selectedPublishableIndex = indexAfter(source, 'isSelectedPublishableImageValue(fields.featuredImage.value)', helperIndex);
    const publishPayloadIndex = indexAfter(source, 'existingFeaturedImagePath: getExistingFeaturedImagePathForPayload(),', selectedPublishableIndex);
    assert.ok(selectedPublishableIndex > helperIndex, 'Existing featured path helper should check whether the selected featured image is publishable.');
    assert.ok(publishPayloadIndex > selectedPublishableIndex, 'Publish payload should use the helper instead of blindly sending the hidden existingFeaturedImagePath value.');
});

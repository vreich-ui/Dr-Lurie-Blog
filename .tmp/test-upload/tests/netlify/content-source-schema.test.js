...
const adminPublishDraftInputWithBody = (bodyPatch) => {
    const base = {
        record_type: 'content_source',
        schema_version: 'content_source.v1',
        content: {
    ...
    return {
        ...
        publication: {
            ...base.publication,
            ...(bodyPatch.publication ?? {}),
            publish_payload: {
                ...base.publication.publish_payload,
                ...(bodyPatch.publication?.publish_payload ?? {}),
            },
        },
    };
};
...
const createWorkflow = (input) => createRequest(createMemoryStore(), {
...
test('create_request rejects admin-publish drafts missing only body text', async () => {
    ...
    assert.deepEqual(body.issues.map((issue) => issue.path.join('.')), ['publication.publish_payload.content']);
...
test('create_request rejects skeletal admin-publish drafts before writing workflow records', async () => {
    ...
    assert.equal(body.ok, false);
    assert.equal(body.error_code, 'invalid_admin_publish_draft');
    assert.ok(body.issues.some((issue) => issue.path.join('.') === 'publication.publish_payload.author'));
    assert.ok(body.issues.some((issue) => issue.path.join('.') === 'publication.publish_payload.content'));
...
test('create_request returns HTTP 400 when required schema discriminator fields are missing', async () => {
    ...
    assert.ok(body.issues.some((issue) => issue.path.includes('record_type')));
    assert.ok(body.issues.some((issue) => issue.path.includes('schema_version')));
});
test('create_request returns HTTP 400 when content_source discriminators are invalid', async () => {
    ...
    assert.equal(response.statusCode, 400);
    assert.equal(body.ok, false);
    assert.ok(body.issues.some((issue) => issue.path.join('.') === 'record_type'));
    assert.ok(body.issues.some((issue) => issue.path.join('.') === 'schema_version'));
});
...
test('publication_status recognizes centralized first-party states while staying open for future values', () => {
    for (const publicationStatus of knownPublicationStatuses) {
        assert.equal(validateContentSourceV1({
            ...validContentSourceV1,
            publication: {
                ...validContentSourceV1.publication,
...
test('create_request returns HTTP 400 for invalid nested publication payloads', async () => {
    ...
    assert.equal(response.statusCode, 400);
    assert.equal(body.ok, false);
    assert.ok(body.issues.some((issue) => issue.path.join('.') === 'publication.publish_payload.title'));
});
...
test('content_source.v1 validates representative agent-generated JSON payloads', async () => {
    ...
    assert.equal(body.record.input.revision_control.revision_requests[0].instruction, 'Tighten intro before final handoff.');
});
test('content_source.v1 rejects unbagged extension fields in concrete agent-priority sections', async () => {
    ...
    assert.equal(response.statusCode, 400);
    assert.equal(body.ok, false);
    assert.ok(body.issues.some((issue) => issue.path.join('.') === 'claims.claim_list.0'));
});

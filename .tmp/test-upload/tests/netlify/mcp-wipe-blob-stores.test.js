...
const callWipeBlobStores = async (args, headers = { 'x-publish-key': 'wipe-secret' }) => {
...
const withMockBlobStores = async (fn) => {
...
test('wipe_blob_stores dryRun returns counts and samples without deleting', async () => {
    await withMockBlobStores(async (stores) => {
        ...
        assert.ok(body.result.structuredContent.sampleKeys?.includes('artifact-index/request-artifacts/request-1/aaaaaaaa.json'));
...
test('wipe_blob_stores requires NETLIFY_PUBLISH_SECRET authorization', async () => {
    await withMockBlobStores(async (stores) => {
        const unauthorized = await callWipeBlobStores({ dryRun: true, prefixes: ['image/'] }, { authorization: 'Bearer wrong-secret' });
        ...
        const authorized = await callWipeBlobStores({ dryRun: true, prefixes: ['image/'] }, { authorization: 'Bearer wipe-secret' });

...
test('admin artifact MCP tools accept PUBLISH_SECRET bearer authorization without Clerk', async () => {
  const previousClerkSecret = process.env.CLERK_SECRET_KEY;
  const previousPublishSecret = process.env.PUBLISH_SECRET;
  const previousNetlifyPublishSecret = process.env.NETLIFY_PUBLISH_SECRET;
  const previousNetlify = process.env.NETLIFY;

  delete process.env.CLERK_SECRET_KEY;
  process.env.PUBLISH_SECRET = 'admin-migration-bypass-secret';
  ...
  try {
    const response = await handler({
      httpMethod: 'POST',
      headers: {
        authorization: 'Bearer admin-migration-bypass-secret',
      ...
      body: JSON.stringify({
        ...
        id: 1,
        method: 'tools/call',
        params: { name: 'migrate_artifact_indexes', arguments: { dryRun: true, limit: 1 } },
      }),
    });
    const body = JSON.parse(response.body) as {
      result: { isError?: boolean; structuredContent: { scanned: number; checkpoint: { totalKeys: number } } };
    };

    assert.equal(response.statusCode, 200);
    assert.equal(body.result.isError, undefined);
    assert.equal(body.result.structuredContent.scanned, 0);
    assert.equal(body.result.structuredContent.checkpoint.totalKeys, 0);
  } finally {
    setNetlifyBlobsModuleForTesting(undefined);
    ...
    if (previousPublishSecret === undefined) delete process.env.PUBLISH_SECRET;
    else process.env.PUBLISH_SECRET = previousPublishSecret;

    if (previousNetlifyPublishSecret === undefined) delete process.env.NETLIFY_PUBLISH_SECRET;
    else process.env.NETLIFY_PUBLISH_SECRET = previousNetlifyPublishSecret;

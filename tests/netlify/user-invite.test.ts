...
const recordingFetch = (ok: boolean, status = ok ? 200 : 500) => {
...
test('invite creates an invited record and fires the GoTrue admin invite', async () => {
  ...
  // GoTrue admin call: right endpoint, bearer token, email in body
  assert.equal(calls.length, 1);
  assert.equal(calls[0].url, 'https://site/.netlify/identity/admin/users');
  assert.equal(calls[0].init.headers.Authorization, 'Bearer admin-token');
  assert.match(calls[0].init.body, /new@example\.com/);
  // persisted

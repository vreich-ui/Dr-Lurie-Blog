...
| W13        | 🟡 BUILT THROUGH THE HUMAN GATE (2026-07-20 — plan [`12-object-tracking-and-analytics.md`](12-object-tracking-and-analytics.md); OQ-W13-1…6 RULED): T13.1–T13.7 MERGED to main (attribute + set_tracking funnel; the ELEVENTH governed type; event schema + /api/t relay + region oracle; loader; render seam; the §8 consent surface; the §7 goal→conversion bridge with google_ads/ga4). T13.8–T13.10 + T13.12/T13.13 BUILT on branch `claude/w13-natives-tail`: the four native pixel adapters (all gated, disabled in-repo) + the site's first CSP (Report-Only, hosts-drift-tested both directions); the owner-DB sink reference kit (receiver contract, schema.sql with pg_notify, mirror-replay script); the trk_drlurie seed + singleton/ten-type drills (local rehearsal all-green; the seed-driver creation identity named; the section-impression gate bug found+fixed); the admin Tracking governance card (OQ-W13-2 surface on the T9.15 override layer); doc 12 §15 scores-feedback design (OQ-W13-5b ANSWER line open). NOTHING CONVERTED — no store record; T13.11 (human_gate) is the ONLY remaining row: env vars per OQ-W13-6, the credentialed `--production --release` drive, live beacon verification, the ten-type set_tracking MCP round-trip, CSP promotion after a clean soak | The 2026-07-19 directive: aggressive-but-legal continuous collection, object-aware and project-scoped — analytics enters the object model (supersedes the config.yaml exclusion) | L    |

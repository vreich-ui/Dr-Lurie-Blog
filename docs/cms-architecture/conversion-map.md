...
| W10        | 🟡 IN PROGRESS (2026-07-19 — see [`11-platformization-plan.md`](11-platformization-plan.md) §1): T10.1+T10.2 BUILT on branch `claude/w10-design-vocabulary` (bounded layout/shape/type token axes on brandTokens — schema/render/apply/validate/contract, byte-identical defaults, build-diff EMPTY); T10.3 survey proposal committed (`design-vocabulary-gaps.md`); T10.4 checkpoint = Wolf ratifies mints/variants/axes before T10.5/T10.6 mint anything                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Design vocabulary: bounded visual range for multi-site theming (rule 6 — enum axes, code-owned mappings)                                                                         | M    |
| W13        | ⚪ PLANNED (2026-07-19 — see [`12-object-tracking-and-analytics.md`](12-object-tracking-and-analytics.md)): tracking as a body attribute on ALL TEN types (`set_tracking`, one-writer funnel) + `tracking_config` as the eleventh governed type (`trk_drlurie`: provider registry with regex-pinned IDs, geo-adaptive consent, per-type collection matrix; export `src/data/site/tracking.json`) + the own first-party tracker (≤4KB loader on the existing `data-cms-*` identity attrs → `/api/t` relay → owner DB, `tracking_event.v1` mirroring commerce_event.v1 rules) + vetted ad adapters (google/meta/natives; plausible dormant; GTM recommended OUT). Briefs T13.1–T13.11; checkpoints OQ-W13-1/-3; production drive human-gated                                                                                                                                                                                                                                                                                                          | The 2026-07-19 directive: aggressive-but-legal continuous collection, object-aware and project-scoped — analytics enters the object model (supersedes the config.yaml exclusion) | L    |

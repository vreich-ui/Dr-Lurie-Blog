...
| W13        | 🟡 IN PROGRESS (2026-07-19 — plan [`12-object-tracking-and-analytics.md`](12-object-tracking-and-analytics.md); OQ-W13-1…6 all RULED): T13.1–T13.5 BUILT on branch `claude/w10-design-vocabulary` — tracking attribute on all ten types + set*tracking (one-writer funnel), `tracking_config` as the ELEVENTH governed type (trk* singleton, regex-pinned providers, GTM permanently out), tracking_event.v1 + /api/t relay (cookieless vhash/shash, geo country+subdivision only, blob mirror), the ≤4KB loader, and the TrackingScripts render seam (byte-invisible until an export exists). NOTHING CONVERTED yet — no store record; T13.10 seeds + T13.11 human-gated drive apply the five criteria                                                                                                                                                                                                                                                                                                                                             | The 2026-07-19 directive: aggressive-but-legal continuous collection, object-aware and project-scoped — analytics enters the object model (supersedes the config.yaml exclusion) | L    |

...
| W13        | 🟡 IN PROGRESS (2026-07-20 — plan [`12-object-tracking-and-analytics.md`](12-object-tracking-and-analytics.md); OQ-W13-1…6 all RULED): T13.1–T13.5 MERGED to main (tracking attribute on all eleven types + set_tracking one-writer funnel; `tracking_config` the ELEVENTH governed type; tracking_event.v1 + /api/t relay with cookieless vhash/shash + region oracle; the loader; the TrackingScripts render seam — byte-invisible until an export exists). T13.6+T13.7 BUILT on branch `claude/w10-mints-w13-consent`: the §8 consent surface (geo-adaptive gating per OQ-W13-1, Consent Mode v2 denied defaults before any vendor head, GPC absolute, ConsentBanner code component with config-copy strings, `_dlconsent`/`_dlid` with the 13-month mint-time cap, #privacy-choices re-open affordance) and the §7 goal→conversion bridge (google_ads always-gated + ga4 adapters, one gtag.js loader rule, build-resolved product_price values, declared-goals-only fan-out, enhanced conversions OFF, v1 goals purchase/opt_in/contact_submit wired). NOTHING CONVERTED yet — no store record; next T13.8 (native adapters + CSP), T13.9, T13.10 seeds, then the T13.11 human-gated drive applies the five criteria         | The 2026-07-19 directive: aggressive-but-legal continuous collection, object-aware and project-scoped — analytics enters the object model (supersedes the config.yaml exclusion) | L    |

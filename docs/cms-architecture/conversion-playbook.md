...
**thirty-one objects are converted**: the three navigation objects, all twelve
page objects (home + about + the 8 W1 interior/system pages + page_contact +
page_thank_you), the twelve shared sections under home/about, the three
templates, the `tax_drlurie` taxonomy registry (W3), and the `site_drlurie`
site singleton (W4) — all proven end-to-end by credentialed
...
| 4   | ~~`content_grid manual item "…" does not resolve to an existing content_item` — always, even for real posts~~ CLOSED 2026-07-11 | The validation context now resolves `content_item` against COMMITTED content (the GitHub contents listing of src/data/post — the W3 source of truth; `netlify/lib/content-item-index.ts`, 60s cache). Unconfigured/erroring lookups degrade to "not verifiable", never to a false failure                                               | Manual curation is LIVE: real post ids (filename minus extension) validate; ghosts are blockers. Render side: a pick deleted AFTER publish is skipped with a build-log warning + fallback backfill — never a build failure                                                                                 |
...
| 13  | Wiring change lights up ~150 pages in build-diff; every hunk is a `<symbol>`↔`<use>` icon flip                                 | An `await` added to previously-SYNC component frontmatter — even on a resolved memoized promise — changes Astro's concurrent sibling evaluation order; a different icon instance now renders first and wins the astro-icon `<symbol>` definition (hit in W4, 2026-07-11)                                                                | Keep wiring loaders SYNCHRONOUS for components that were sync: eager `import.meta.glob` on the derived export (zero-or-one match; absent → fallback), not an async `getEntry` — see `src/utils/site-object.ts`                                                                                             |
| 14  | EVERY production deploy fails at "Secrets scanning found secrets in build" naming a derived export                              | An agent put a URL containing the repo slug into object content (hit 2026-07-11: `portrait.src` = weserv proxy of raw.githubusercontent.com/(repo)/…) — the slug is the VALUE of secret-marked `GITHUB_REPOSITORY`, and the scanner matches it (even URL-encoded) in repo files AND build output, blocking ALL deploys until it is gone | Heal through the STORE, never the export (one `update_section_data` deep-merge of the field; `scripts/fix-about-portrait.mjs` is the template), publish + release. Image URLs in content must be site assets (the kugelmedia host or a first-party `/images/…` path) — never repo files or proxies of them |

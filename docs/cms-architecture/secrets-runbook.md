...
| `PDF_TOOL_BASE_URL` / `PDF_TOOL_AGENT_RUN_TOKEN` | fleet-shared | Server-side Platform artifact bridge endpoint and bearer. The bearer authorizes Platform to call pdf-tool; it never enters MCP responses or logs. | Netlify env var, Functions scope | Rotate with pdf-tool's `AGENT_RUN_TOKEN`, update every site deployment, then run the create → poll → verify bridge test. |

...
- **OQ-2 — Scheduling machinery: go-live rebuilds AND pending exports.** Two coupled problems. (a) The visibility gate is build-time (A§2.5, §4.3); a future `published_time` only goes live when something rebuilds the site — today articles have the same latent issue (A§1.7). (b) The single-export model cannot express a scheduled *update* to an already-live object (§5.6 scheduling semantics) — that needs pending-path materialization promoted at go-live. Both want the same scheduler (Netlify scheduled builds/build hooks or a scheduler function); until it exists, `publish(id, future)` is first-publish-only and live-object updates are immediate-only. Unresolved; affects ops more than schema.

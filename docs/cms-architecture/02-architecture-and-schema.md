...
interface NavItem {
  ...
  icon?: string;                                      // M-7: icon-set name (e.g. 'tabler:rss')
  ariaLabel?: string;                                 // M-7: social links carry both today
                                                      // (navigation.ts:104, A§2.3) — without
                                                      // them the P2 footer cutover loses the
                                                      // RSS icon and its accessibility label
}
type NavTarget =
...
schedule  ≡ publish(id, future) — FIRST publishes only; see scheduling semantics below (OQ-2)
```

Failure semantics of the ordering — **stated separately for publish and unpublish, because the residuals point in opposite directions:**

*Publish:* a failure at or before step 4 leaves both the record and the repo unchanged — the record never claims a publish that didn't happen. A failure between steps 4 and 5 leaves the export committed but the record un-stamped: the residual window today's article path already has (A§1.6), and the safe direction — the source *under*-claims, and retrying is idempotent because re-publish overwrites the same export. The forbidden state (record stamped published, no committed export) cannot occur by construction **for publish**.

*Unpublish (`publish(id, null)`):* the same export-first order is kept deliberately — the alternative (stamp null first, remove export second) would leave content **live on the site after a failure**, which is the worse outcome for the takedown intent that motivates most unpublishes. The cost is the inverse residual: a failure between steps 4 and 5 leaves the export removed/neutralized while the record still carries the old non-null `published_time` — the record *over*-claims, the site under-shows. This state is (a) reader-safe (the content is already off the site, which is what the operator wanted first), (b) detectable (no unpublish receipt on the record while the committed export is gone — the same divergence check the diagnostics endpoint family exists for), and (c) repaired by mandatory idempotent retry: re-running the unpublish re-commits a no-op removal and completes the step-5 stamp. The invariant for unpublish is therefore not "the mismatch cannot occur" but "the mismatch is always in the reader-safe direction and must be retried to convergence" — an operation that fails step 5 is surfaced as incomplete, never silently dropped.

Scheduling semantics (constrained by the single-export model): there is exactly one derived export per object, and visibility is a build-time gate on `published_time` (§4.3). That combination handles a **first** publish with a future timestamp correctly — the export is committed, the gate hides it until the time arrives (precisely how future-dated articles already behave, A§1.7). It **cannot** express a scheduled *update* to an already-published object: step 4 would overwrite the live export immediately, after which builds either hide a currently-live surface (gated types) or show the future content early (ungated chrome like navigation). Therefore `publish(id, future)` is **valid only when the object is currently unpublished — and only for object types whose consumers actually apply the timestamp gate**: `content_item` (the blog.ts gate, A§1.7) and `page` (the §4.3 route-loader gate). Navigation, site, taxonomy, template, and shared-section exports are consumed as chrome/reference data the moment they are committed — no loader checks their `published_time` — so even a *first* future-dated publish of `nav_header` or the Site object would go live at the next build, early. The gate rejects future timestamps for those types outright; their publishes are immediate or `null`. (Gating shared-section dereferences and chrome data at build time is possible additional machinery, folded into **OQ-2** with the rest of scheduling.) Scheduled *updates* to live objects of any type additionally require pending-export machinery — materialize to a pending path, promote at go-live — the same scheduler problem as the rebuild trigger, likewise OQ-2. Until OQ-2 lands: future timestamps = first publishes of gated types only; everything else is immediate-only.
...
- **OQ-2 — Scheduling machinery: go-live rebuilds AND pending exports.** Two coupled problems. (a) The visibility gate is build-time (A§2.5, §4.3); a future `published_time` only goes live when something rebuilds the site — today articles have the same latent issue (A§1.7). (b) The single-export model cannot express a scheduled *update* to an already-live object (§5.6 scheduling semantics) — that needs pending-path materialization promoted at go-live. Both want the same scheduler (Netlify scheduled builds/build hooks or a scheduler function); a third sub-problem is that chrome/reference exports (navigation, site, taxonomy, template, shared sections) have no timestamp gate at all, so scheduling them needs a gate or pending-path even for first publishes (§5.6). Until OQ-2 lands, `publish(id, future)` is first-publish-of-gated-types-only (`content_item`, `page`) and everything else is immediate-only. Unresolved; affects ops more than schema.

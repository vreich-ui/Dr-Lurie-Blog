...
console paths. S2's planned audit script (`scripts/audit-site-admin-parity.mjs`)
never landed, so this audit was done by hand against
`packages/core/cli/create-site.mjs`'s emissions; the drift that IS now
guarded in CI is called out per tenant below.

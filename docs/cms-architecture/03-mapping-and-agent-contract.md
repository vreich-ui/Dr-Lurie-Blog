...
**Mapping-discovered schema amendment (M-7):** the social entry above carries presentation/accessibility metadata — `icon: 'tabler:rss'` and `ariaLabel: 'RSS'` (`navigation.ts:104`), which `Footer.astro` renders as an icon-only link (A§2.3) — but `NavItem` preserves only label/target/children. Amend D§3.8: `NavItem.icon?: string; ariaLabel?: string`. Without them the P2 footer cutover cannot feed `Footer.astro` its icon/aria-label props and the byte-identical footer diff fails.
...
5. **The mobile-only "Join Newsletter" CTA** hardcoded in `Header.astro:210-216` (A§2.2), rendered only inside `md:hidden` mobile utilities. `NavigationBody.actions` has no viewport scoping, and adding responsive variants to nav data is presentation leaking into structure. Options: (a) accept it as a normal action — **note: this changes the desktop header** (the CTA appears there for the first time), so it is an intentional behavior change, not a migration default; (b) drop it (newsletter is reachable via nav); (c) add `LinkAction.visibility?: 'all'|'mobile'|'desktop'`. **Flagged as open (labelled OQ-11)**; the only *diff-safe* interim is to leave the CTA hardcoded in Header chrome until Wolf picks (a)/(b)/(c) — see the P2 plan for the migration consequence. The contract below is unaffected by the choice.
...
Mapping verdict: every editorial surface the audit found lands on an existing concept; six small schema amendments (M-1 item descriptions, M-2 group slots, M-3 `form_thank_you`, M-4 chrome search, M-5 group-level targets, M-7 social icon/aria metadata — M-6 is a contract-side amendment defined in §2.2) and one new open question (OQ-11 viewport-scoped actions) fall out. Nothing required a new top-level concept — the D§2 set held.
...
- **Tier 2 — `page` / `section` / `template`: agent may execute publish, but only with an approval pinned to the current `content_revision` *and* to the reviewed publish action** (D§3.9, amended). A content-only pin would be a hole: the agent supplies `published_time` at call time, so with a mere content approval it could publish immediately, schedule an arbitrary time, or pass `null` to *unpublish* — none of which the reviewer saw. **Amendment M-6 to D§3.9:** `object_submit_review` carries `requested_publish_action: {published_time: ISO | null | 'immediate'}` whenever a Tier 2 agent-executed publish is intended; approval pins `{content_revision, publish_action}` together; the agent's `object_publish_by_time` call must match the pinned action exactly (`'immediate'` = the call omits `published_time`; an ISO pin must equal the supplied timestamp; a `null` pin is the only thing that authorizes an agent unpublish). Any different action requires re-approval. Humans holding a publish role are not constrained by the pin — they carry publish authority in themselves; the pin constrains delegated execution. With that in place, "approve now, go live at 9am" works because *9am was part of what was approved* — noting D§5.6's scheduling constraint: a future-time pin is only executable for a **first** publish (unpublished object) **of a timestamp-gated type** — among Tier 2 types that means `page` only (navigation/site/taxonomy/template/shared-section exports are consumed ungated once committed, D§5.6); scheduled updates to live objects are rejected until OQ-2's pending-export machinery exists. So in practice the pinnable actions are: `immediate`/`null` for any Tier 2 object, plus a future ISO time only for an unpublished `page`.
...
- **Safe (additions):** slug unique per kind; slug shape `^[a-z0-9]+(?:-[a-z0-9]+)*$` (the existing slug rule, A§1.6); `merged_into` targets exist, are `active`, and form no cycle; deprecating a term with live usage *requires* `merged_into` (no stranding — the A§2.6 rename-strands-posts failure made structurally impossible). **Slug renames never happen in place:** because content items keep free strings that resolve *by slug* (D§5.5), an in-place slug change would silently orphan every stored occurrence of the old string. `update_term` with a changed `slug` therefore atomically mints a deprecated alias entry `{new term_id, slug: <old slug>, status:'deprecated', merged_into: <this term>}` in the same op — the old string keeps resolving through the standard alias machinery with no special cases. An `update_term` that changes `slug` without the alias is rejected by validation.
...
- **OQ-2 (scheduled publish).** `object_publish_by_time` with a future timestamp is allowed wherever publish is allowed, **and only when the object is currently unpublished, and only for timestamp-gated object types (`content_item`, `page`)** — navigation/site/taxonomy/template/shared-section exports are consumed ungated once committed, so a scheduled first publish of chrome/reference data would go live at the next build instead of the requested time; those types are immediate-or-`null` (D§5.6 scheduling semantics — additionally, the single-export model cannot hold a live export and a scheduled update simultaneously, so live-object updates of any type are immediate-only until OQ-2's pending-export machinery lands); *going live* still depends on a rebuild at/after that time. The contract records the receipt with `deployStatus` so an agent can detect "stamped but not yet built" and re-trigger — but the rebuild/promotion mechanism itself remains OQ-2.
...
| Non-fitting findings flagged, not forced | §1.7 (7 items), amendments M-1…M-7, OQ-11 |

...
- **OQ-2 (scheduled publish).** `object_publish_by_time` with a future timestamp is allowed wherever publish is allowed **and the object is currently unpublished** (D§5.6 scheduling semantics — the single-export model cannot hold a live export and a scheduled update simultaneously; live-object updates are immediate-only until OQ-2's pending-export machinery lands); *going live* still depends on a rebuild at/after that time. The contract records the receipt with `deployStatus` so an agent can detect "stamped but not yet built" and re-trigger — but the rebuild/promotion mechanism itself remains OQ-2.

...
| T2.9 Solutions dedupe      | **CHECKPOINT — genuinely open, your call**                                                                      | §8                                                                                   |
...
**One rendering detail interacts with T2.9 and is worth ten seconds of your
attention at execution time:** `Header.astro` today renders no `actions` at
all — the seeded 'Join Early Access' action is faithful-but-dead data. Once
the header renders actions from data, _both_ actions appear unless the
'Join Early Access' action is removed (which is exactly one of the T2.9
options). Sequencing T2.9's answer before T2.8 avoids deciding this twice.
...
## 8. T2.9 — CHECKPOINT (open question for Wolf, never pre-decided)

The seeded nav faithfully carries three links to `/solutions/early-access`:
the 'Early Access' item, the 'Join Early Access' item (both in the Solutions
dropdown), and the 'Join Early Access' header action. **Question: remove one
of the three (which?), or keep all three?** If dedupe: one reviewed
`remove_item` (or `remove_action`) op through the standard flow, two minutes.
If keep: nothing to do; the duplicate-target warning remains warn-only by
design. See also the §7 interaction note.

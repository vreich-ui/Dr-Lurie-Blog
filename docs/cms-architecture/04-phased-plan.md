...
**Rollback safety:** single revert of the cutover commit restores `navigation.ts` consumption (the file is deleted only in this phase's *cleanup step, after* the cutover has survived a production deploy; until then it remains in-tree, unreferenced). Derived JSON is inert when unreferenced.
...
**Explicit data resolution (no silent clean-data assumption):** the "Start here" grid placeholders (C§1.7-1) are **invalid under the contract** (`content_grid` manual items must reference real content items, C§2.3-page). This phase therefore contains a scheduled decision task for Wolf — (a) designate/write five real articles, or (b) switch to `source:{kind:'query'}` — and the seed carries the current placeholder *copy* in a `hidden`-visibility section until decided, so the live page keeps today's appearance while the draft records the unresolved state honestly. The phase's exit criteria do not require the decision; publishing any *change to that section* does.
...
**Exit criteria:** built-HTML diff empty for `/`; agent flow of C§2.5 example A (hero copy) and example B (reorder + hide, with a Discard-inverse exercised) run end-to-end against production; a `testimonial` registry module can be added and used on a draft without touching any file outside the registry (proves the "add a testimonial section" promise mechanically).
...
- **Template-leftover deletion** (A§2.9: `homes/*`, `landing/*`, `services`, `pricing`, `LandingLayout`, and the unused `CallToAction`-widget usages): safe to delete once no real page depends on widget files being present; scheduled here (not earlier) so deletions never interleave with cutovers of pages that still render.
...
**Exit criteria:** every object type in the C§2.2 matrix exists in production with its contract tier enforced; the Deferred register (below) is empty or re-triaged.
...
|---|---|---|
| 1. Homepage grid placeholders | **P3** | Carried as hidden-visibility seed + scheduled Wolf decision (real articles vs. query); publish-blocked until decided |
...
2. **Nav targets before Pages exist.** The contract validates `NavTarget.page` refs against Page objects (C§2.0 item 3), but P2 ships navigation before P3 creates any Page. The plan bridges with `route`-kind targets in P2, upgraded in P3. C§ didn't specify this ordering wrinkle; recorded here so the P2 seed doesn't get built to an impossible validation rule.

...
- **Template-leftover deletion** (A§2.9: `homes/*`, `landing/*`, `services`, `pricing`, and the unused `CallToAction`-widget usages — *minus* `LandingLayout.astro` and `homes/saas.astro`, already deleted in P2's cleanup because they import `navigation.ts`): safe to delete once no real page depends on widget files being present; scheduled here (not earlier) so deletions never interleave with cutovers of pages that still render.

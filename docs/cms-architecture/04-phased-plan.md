...
**Exit criteria:** built-HTML diff of header/footers vs. pre-cutover build is empty (modulo attribute ordering); an agent `update_item` on a footer CTA label flows through structural/field review to a human-executed publish and appears on the live site; `navigation.ts` deleted; OQ-7 **settled by Wolf**: `nav_footer_home` is a permanent, distinct Navigation instance (kept, not merged into `nav_footer`) — the seed builds it as the intended end-state, not a migration artifact (05 §1).

**§1.7 items handled here:** #4 (duplicate 'Early Access' targets → carried faithfully, warn-not-reject, editorial cleanup offered); #5/OQ-11 (mobile-only newsletter CTA → **SETTLED by Wolf, no checkpoint**: option (a), a plain action identical on all devices, with **no viewport field added to the schema** — part of a standing project principle of zero device-conditional content structure (05 §1). Migration mechanics unchanged by the settlement: the CTA stays hardcoded in Header chrome through the byte-identical cutover — it renders only inside `md:hidden` mobile utilities today (`Header.astro:184-216`), so applying (a) at cutover would change the desktop header and break the empty diff — and the settled decision is then applied as an intentional post-cutover nav edit + chrome removal, one named task in 05's P2 breakdown).
...
**Scope (concepts):** PageType registry v1 (`home`, `standard`, `system` stubs — D§3.4); component registry v1 with exactly the types the homepage needs: `hero`, `checklist`, `content_grid`, `bio`, `newsletter_signup`, `shared_ref` (D§3.5, C§1.1) — implementations extracted from the existing `index.astro` markup (the `dl-*` idiom becomes registry components, D§2.6); shared section `sec_newsletter_signup` (first shared object, with reference counting live — C§2.3-section); `page_home` seeded per C§1.1 with verbatim content — with one seeding adjustment: the hero's `LinkAction`s target `page_start_here` and `page_newsletter` (C§1.1), which don't exist until P4, and reference integrity rejects unresolvable `page` refs at patch time (C§2.0); those actions are therefore seeded as the transitional `{kind:'route'}` targets (the same P0-added variant the nav bridge uses, applicable anywhere a `NavTarget` appears) and upgraded to `page` refs in P4; renderer boundary per D§4.2–4.3 (`data`/`resolved`/`ctx` props, visibility filtering, published_time gate extended to pages); Tier 2 publish flow live (agent publish after pinned approval); full impact preview (surface 3) now computable (pages referencing shared sections); P2's interim footer-instance hardcode replaced by `page_home.navigationOverrides.footer`; nav `route` targets upgraded to `page` targets for pages that now exist.

**Explicit data resolution (no silent clean-data assumption):** the "Start here" grid placeholders (C§1.7-1) are **invalid under the contract** (`content_grid` manual items must reference real content items, C§2.3-page) — and reference-integrity checks reject such data at patch time, not just publish time (C§2.0), so the placeholder state cannot even be *seeded* as a manual list. It also cannot be hidden away: the renderer filters by visibility (D§4.3), so a hidden section would drop the grid from the built page and break this phase's empty-HTML-diff requirement. Therefore: **the Wolf decision is now SETTLED — manual-primary with query fallback** (default manual curation; when the curated list has fewer items than the section `limit`, backfill from a query — a `content_grid.source` schema refinement recorded as amendment **M-8** in 05 §1; note 05's flag that the settlement brief called it "M-6", a number already taken by the publish-action pin). The cutover mechanics below are unchanged by the settlement, because applying it at cutover would swap placeholder cards for real articles and break the empty diff: the seed still uses the transitional static-cards variant for byte-identical output, and the settled manual+fallback configuration is applied immediately after as an intentional post-cutover change through Tier 2 review (named task in 05's P3 breakdown). The transitional variant is `source:{kind:'static', cards:[{title, description}]}`: it renders the audited placeholder copy verbatim (byte-identical output), is publish-valid, is marked deprecated-on-arrival, and is retired by the post-cutover switch task. This path keeps the live page unchanged at cutover and never smuggles the invalid manual-refs state into the store.
...
|---|---|---|
| 1. Homepage grid placeholders | **P3** | SETTLED: manual-primary + query fallback (amendment M-8, 05 §1); static-cards transitional seed keeps the cutover byte-identical, then the settled configuration is applied as an intentional post-cutover change — never hidden, never seeded as invalid manual refs |
...
| 5. Mobile-only newsletter CTA (OQ-11) | **P2** | SETTLED: plain action on all devices, no viewport schema field (standing principle, 05 §1); stays hardcoded chrome through the byte-identical cutover, then applied as an intentional post-cutover change |
...
2. **Nav targets before Pages exist.** The contract validates `NavTarget.page` refs against Page objects (C§2.0 item 3), but P2 ships navigation before P3 creates any Page — and D§3.8's `NavTarget` union has no variant that could express "this route, no object yet." The plan therefore **adds a transitional schema variant** rather than shipping schema-invalid seeds or dragging Page stubs into P2: `{kind:'route', href}` enters the validators in **P0** and is legal *anywhere a `NavTarget` appears* — nav items and groups **and page-body `LinkAction`s** (the P3 homepage hero targets pages that only exist in P4, so it needs the same bridge). It is the seed form for page-like targets in **P2** (nav) and **P3** (homepage actions), upgraded to `page` refs as the referenced pages materialize (**P3** for the homepage-adjacent set, **P4** for the rest, **P6** for 'Topics'/listing pages — the last consumers), and is **removed from the validators in P6 cleanup**, whose exit criteria require zero `route`-kind targets in any published object. C§ didn't specify this ordering wrinkle; recorded here so the P2 seed doesn't get built to an impossible validation rule.

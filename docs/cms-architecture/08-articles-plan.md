...
| **W7.7** | Admin editor on rich text + annotation panel; single renderer everywhere; bugs ⑥⑩.                                                                                                                                                                                                                | Suite; editor drive                                      | ⚙ CANVAS CAPABILITY SLICE SHIPPED (2026-07-13, 07-canvas §3h): node palette (incl. offer/affiliate + mock-ad + gallery starters), adSlot mockup bank, the visible ROLE/annotation panel (strategy/intent/notes per block), multi-image blocks, and the missing `upsert_node` id-mint. REMAINDER ON HOLD (Wolf, 2026-07-13 second field test): the old admin-editor UI is stale — Wolf is rethinking what the admin area should be; no TipTap panel work or /admin/publish re-wire until that ruling. Bugs ⑥⑩ remain tabled. |
